...
class SelfAttention_Model(nn.Module):
	def __init__(self, 
		...
		self.conv_extractor = nn.Sequential(nn.Conv1d(self.in_fc, 512, 1),
											...
											nn.Conv1d(self.fc1, self.fc2, 1),
											nn.PReLU(),)
		self.f = nn.Conv1d(self.fc2, self.fc2, 1)
		self.g = nn.Conv1d(self.fc2, self.fc2, 1)
		self.h = nn.Conv1d(self.fc2, self.fc2, 1)
		# Choose scoring or Pooling layer
		#self.scorer = nn.Conv1d(self.fc2, 1, 1)
		#self.fc_in = 
		# Choose pooling layer
		self.pool = nn.AvgPool1d(kernel_size = 10000)
		self.fc_in = self.fc2
		# The input channel depends on the previous layer
		self.classifier = nn.Sequential(nn.Linear(self.fc_in, self.output))
	...
	def forward(self, x): 
		...
		x_fg = torch.matmul(x_f.T, x_g)
		x_fgs = F.softmax(x_fg, dim = -1)
		x = torch.matmul(x_h, x_fgs)
		print(x.size())

		# if use the pooling layer
		x = torch.transpose(x,1,0)
		x = self.pool(x)
		x = torch.transpose(x,1,0)
		y_prob = self.classifier(x)
